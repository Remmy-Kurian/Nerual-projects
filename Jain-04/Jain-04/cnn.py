...
class CNN(object):
    ...
    def add_input_layer(self, shape=(2,),name="" ):
        """
         ...
         :return: None
         """
        inp = layers.InputLayer( input_shape=shape, name=name)
        self.model.add(inp)
    ...
    def append_dense_layer(self, num_nodes,activation="relu",name="",trainable=True):
        """
         ...
         """
        act =activation
        a = layers.Dense(num_nodes, activation=act, name = name,  trainable = trainable)
        self.model.add(a)
#         self.layers.append(name)
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1,
        ...
        p = padding
        act = activation
        l = layers.Conv2D(num_of_filters, kernel_size, strides= s , padding=p, activation=act, name = name, trainable = trainable)
        self.model.add(l)
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2,name=""):
        ...
        s = strides
        p = padding
        l = layers.MaxPool2D(pool_size=pool_size, strides=s, padding=p, name=name)
        self.model.add(l)
#         self.layers.append(name)
        return self.model.layers[-1]
    ...
    def set_training_flag(self,layer_numbers=[],layer_names="",trainable_flag=True):
        ...
        if layer_numbers:
            for layer_no in layer_numbers:
                self.model.get_layer(index=layer_no).trainable = tf
        elif layer_names:
            if type(str) == list:
                for name in layer_names:
                    self.model.get_layer(name=name).trainable = tf
            else:
                self.model.get_layer(name=layer_names).trainable = tf
        else:
            return None
...
#             model.compile(loss=self.loss)
    ...
    def set_metric(self,metric):
        ...
        self.metric = metric
#         if metric == "accuracy":
#             self.metrics = keras.metrics.Accuracy(name='accuracy')
#         if metric == "mse":
#             self.metrics = tf.keras.metrics.MeanSquaredError(name='mean_squared_error')
...
#         bs = batch_size
#         e = num_epochs
#         self.model.compile(optimizer=self.optimizer, loss=self.l  metrics=self.metrics)
#         history = self.model.fit(X_train, y_train, batch_size=bs, epochs=e, verbose=2, shuffle=True)
